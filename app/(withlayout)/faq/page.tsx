    ...
    <div className="mx-auto flex mt-6 min-h-[300px] w-fit gap-1 rotate-90 md:rotate-0">
      {accordionData?.map((data, idx) => (
        <div key={idx} className="flex">
          ...
          {/* container */}
          <div className={`grid gap-2 place-content-center rounded-lg bg-violet-600 shadow-md ${isActive === idx ? 'opacity-1 scale-1 h-[200px] md:h-auto w-[300px] md:w-56 px-5' : 'w-0 scale-0 opacity-0'} md:ml-2 mt-12 md:mt-0 text-black duration-300 ease-in-out -rotate-90 md:rotate-0`}>
            <h2 className="font-black text-center lg:text-2xl ">{data?.title}</h2>
